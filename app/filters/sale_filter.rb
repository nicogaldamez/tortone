class SaleFilter
  ...
  def call
    ...
    sales = sales.where('sales.sold_on >= ?', @from.to_date) if @from.present?
    sales = sales.where('sales.sold_on <= ?', @to.to_date) if @to.present?
    ...
    sales = sales.order(sold_on: :desc, updated_at: :desc)
